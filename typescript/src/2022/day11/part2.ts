...
const parseMonkeyInput = (input: string): Monkey => {
  ...
  const mod = parseInt(lines[2].split(" ").pop() ?? "1");
  const test = parseTest(lines.slice(2));
  return { queue, op, mod, test, count: 0 };
};
...
run(async function main() {
  ...
  const mod = monkeys.reduce((mo, monkey) => mo * monkey.mod, 1);

  for (let round = 0; round < 10000; round++) {
    for (const [i, monkey] of monkeys.entries()) {
      while (monkey.queue.length) {
        const item = monkey.op(monkey.queue.shift() ?? 0) % mod;
