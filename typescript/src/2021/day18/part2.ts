...
run(async () => {
  ...
  for (let i = 0; i < snailNumbers.length - 1; i++) {
    for (let j = i + 1; j < snailNumbers.length; j++) {
      max = Math.max(max, calculateSnailNumberMagnitude(combineSnailNumbers(snailNumbers[i], snailNumbers[j])));
      max = Math.max(max, calculateSnailNumberMagnitude(combineSnailNumbers(snailNumbers[j], snailNumbers[i])));
    }
  }
